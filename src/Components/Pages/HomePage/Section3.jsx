...
const Section3 = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (
          ...
          response.data.data.homePage.section3
        ) {
          const section2Data = response.data.data.homePage.section3;
          ...
          setSelectedImage(`${baseUrl}${section2Data.backgroundImageUrl}`);
  ...
  const handleCancel = () => {
    if (originalData) {
      ...
      setSelectedImage(`${baseUrl}${originalData.backgroundImageUrl}`);
  ...
  return (
    <div className="max-w-lg ml-[2rem] mt-[2rem]">
      ...
      <div className="border border-l border-gray m-[2rem] w-[290%] "></div>
    </div>
  );
};
