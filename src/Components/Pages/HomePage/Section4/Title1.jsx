...
const Section4 = ({ index, item, fetchData }) => {
  ...
  useEffect(() => {
    ...
    setDescription(section4DataItem1.description);
    setOriginalData(section4DataItem1); // Store original fetched data
    setBackgroundImageUrl(`${section4DataItem1.backgroundImageUrl}`);
    setSupportiveImages(section4DataItem1.supportiveImages);
    setEditorContent(section4DataItem1.description);
  }, [item]);
          ...
          <div className="hidden md:flex items-center justify-between mb-[1rem]">
            ...
            {loading ? (
              <CircularProgress
                size={24}
                color="inherit"
                className="absolute ml-[70%]"
              />
            ) : (
              <button
                className="text-black bg-white border-2 border-black rounded-2xl px-3 py-2 absolute ml-[70%]"
                onClick={removeSlide}
              >
                Remove Slide
              </button>
