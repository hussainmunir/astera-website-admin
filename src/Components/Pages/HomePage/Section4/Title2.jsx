...
const Title2 = ({ fetchData }) => {
  ...
  const onDropSupportiveImages = useCallback(
    (acceptedFiles) => {
      setSupportiveImages((prevImages) => [...prevImages, ...acceptedFiles]); // Concatenate new images with existing array
    },
    [setSupportiveImages]
  );
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(
        `${baseUrl}homescreen/addSection4Item`,
        formData,
        {
          headers: {
      ...
      <div>
        <div className="max-w-lg ml-[2rem] mt-[2rem]">
          <div className="hidden md:flex items-center justify-between mb-[1rem]">
            ...
            <button
              className="text-black bg-white border-2 border-black rounded-2xl px-3 py-2 absolute ml-[80%]"
              onClick={resetState}
            >
              Cancel
            </button>
