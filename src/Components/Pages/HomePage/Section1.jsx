		...
		const fetchData = async () => {
			try {
				...
				if (
					...
					const section1Data = response.data.data.homePage.section1[0];
					setTitle(section1Data.title);
					setSubtitle(section1Data.subtitle);
					setDescription(section1Data.description);
					setOriginalData(section1Data); // Store original fetched data
					setSelectedImage(
						`https://backend.asteraporcelain.com/${section1Data.backgroundImageUrl}`
					);
					setEditorContent(section1Data.description);
				}
			} catch (error) {
		...
		<div className="max-w-lg ml-[2rem] mt-[2rem]">
			<div className="flex items-center justify-between mb-[1rem]">
				...
				{loading ? (
					...
					<button
						className="text-white bg-purple-600 rounded-lg px-5 py-2.5 absolute top-[40rem] ml-[90%]"
						onClick={handleSave}
					>
						Save
					</button>
				...
				<button
					className="text-black bg-white border-2 border-black rounded-2xl px-3 py-2 absolute top-[40rem] ml-[85%]"
