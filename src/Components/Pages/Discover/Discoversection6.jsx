...
export function Discoversection6() {
    ...
    const fetchSectionData = async () => {
      try {
        ...
        if (
          ...
          response.data.data.discoverPage.section6
        ) {
          const { section6 } = response.data.data.discoverPage;
          setSectionData(section6);
          setEditorContent(section6.description);
          // Set the title state here
          setSectionTitle(section6.title);
        }
      } catch (error) {
      ...
      {sectionData && (
        <div className="max-w-lg ml-[2rem] mt-[2rem]">
          ...
          {loading ? (
            ...
            <button
              className="text-white bg-purple-600 rounded-lg px-5 py-2.5 absolute top-[230rem] ml-[86%]"
              onClick={handleSave}
            >
              Save
            </button>
          ...
          {saveSuccess && (
            <div className="text-green-600 mt-2 absolute top-[235rem] ml-[80%]">
              Save successful!
            </div>
          )}
          <button
            className="text-black bg-white border-2 border-black rounded-2xl px-3 py-2 absolute top-[230rem] ml-[80%]"
            onClick={handleCancel}
          >
            Cancel
          </button>
          {resetMessage && (
            <div className="text-red-600 mt-2 absolute top-[235rem] ml-[85%]">
              {resetMessage}
            </div>
          )}
