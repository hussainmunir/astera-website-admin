...
export function Discoversection5() {
		...
		const fetchSectionData = async () => {
			try {
				...
				if (
					...
					response.data.data.discoverPage.section5
				) {
					const { section5 } = response.data.data.discoverPage;
					setSectionData(section5);
					setEditorContent(section5.description);
					// Set the title state here
					setSectionTitle(section5.title);
				}
			} catch (error) {
	...
	const handleSave = async () => {
		...
		try {
			const response = await axios.post(
				"https://backend.asteraporcelain.com/api/v1/discoverScreen/updateSection5",
				requestData,
				{
					headers: {
			...
			{sectionData && (
				<div className="max-w-lg ml-[2rem] mt-[2rem]">
					...
					{loading ? (
						...
						<button
							className="text-white bg-purple-600 rounded-lg px-5 py-2.5 absolute top-[180rem] ml-[86%]"
							onClick={handleSave}
						>
							Save
						</button>
					...
					{saveSuccess && (
						<div className="text-green-600 mt-2 absolute top-[185rem] ml-[85%]">
							Save successful!
						</div>
					)}
					<button
						className="text-black bg-white border-2 border-black rounded-2xl px-3 py-2 absolute top-[180rem] ml-[80%]"
						onClick={handleCancel}
					>
						Cancel
					</button>
					{resetMessage && (
						<div className="text-red-600 mt-2 absolute top-[185rem] ml-[85%]">
							{resetMessage}
						</div>
					)}
