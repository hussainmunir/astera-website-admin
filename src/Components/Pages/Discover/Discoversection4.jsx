		...
		const fetchSectionData = async () => {
			try {
				...
				if (
					...
					response.data.data.discoverPage.section4
				) {
					const { section4 } = response.data.data.discoverPage;
					setSectionData(section4);
					setEditorContent(section4.description);
					// Set the title state here
					setSectionTitle(section4.title);
				}
			} catch (error) {
	...
	const handleSave = async () => {
		...
		try {
			const response = await axios.post(
				"https://backend.asteraporcelain.com/api/v1/discoverScreen/updateSection4",
				formData,
				{
					headers: {
			...
			{sectionData && (
				<div className="max-w-lg ml-[2rem] mt-[2rem]">
					...
					{loading ? (
						...
						<button
							className="text-white bg-purple-600 rounded-lg px-5 py-2.5 absolute top-[140rem] ml-[90%]"
							onClick={handleSave}
						>
							Save
						</button>
					...
					{saveSuccess && (
						<div className="text-green-600 mt-2 absolute top-[145rem] ml-[85%]">
							Save successful!
						</div>
					)}
					<button
						className="text-black bg-white border-2 border-black rounded-2xl px-3 py-2 absolute top-[140rem] ml-[85%]"
						onClick={handleCancel}
					>
						Cancel
					</button>
					{resetMessage && (
						<div className="text-red-600 mt-2 absolute top-[145rem] ml-[85%]">
							{resetMessage}
						</div>
					)}
