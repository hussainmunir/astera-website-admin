...
export function Discoversection3() {
		...
		const fetchSectionData = async () => {
			try {
				...
				if (
					...
					response.data.data.discoverPage.section3
				) {
					const { section3 } = response.data.data.discoverPage;
					setSectionData(section3);
					setEditorContent(section3.description);
					// Set the title state here
					setSectionTitle(section3.title);
				}
			} catch (error) {
			...
			{sectionData && (
				<div className="max-w-lg ml-[2rem] mt-[2rem]">
					...
					{loading ? (
						...
						<button
							className="text-white bg-purple-600 rounded-lg px-5 py-2.5 absolute top-[100rem] ml-[90%]"
							onClick={handleSave}
						>
							Save
						</button>
					...
					<button
						className="text-black bg-white border-2 border-black rounded-2xl px-3 py-2 absolute top-[100rem] ml-[85%]"
